...
namespace Tweetinvi
{
    public static class TwitterAccessorAsync
    {
        ...
        public static async Task<IEnumerable<T>> ExecuteCursorGETQuery<T>(
            string query,
            int maxObjectToRetrieve = Int32.MaxValue,
            long cursor = -1)
            where T : class, IBaseCursorQueryDTO
        {
            return await Sync.ExecuteTaskAsync(() => TwitterAccessor.ExecuteCursorGETQuery<T>(query, maxObjectToRetrieve, cursor));
        }
